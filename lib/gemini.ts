...
export const callGemini = async (prompt: string, imageBase64?: string, systemInstruction?: string): Promise<string> => {
  ...
  console.log('Gemini URL:', `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${process.env.NEXT_PUBLIC_GEMINI_API_KEY?.slice(0,10)}`);
